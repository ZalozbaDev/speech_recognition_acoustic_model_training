...
cat $dir/lexicon_words.txt | awk '{ for(n=2;n<=NF;n++){ phones[$n] = 1; }} END{for (p in phones) print p;}' | \
  sort | sed '/\./d' > $dir/nonsilence_phones.txt

( echo "." ; echo usb ) > $dir/silence_phones.txt
...
echo '<unk> usb' | cat - $dir/lexicon_words.txt | sort | uniq > $dir/lexicon.txt
